...
const CC_VARS = [
  'CODEX_OCTO_BOT_TOKEN', 'CODEX_OCTO_API_URL', 'CODEX_OCTO_CWD', 'CODEX_OCTO_CWDBASE',
  'CODEX_OCTO_DATA_DIR',
  'CODEX_OCTO_SDK_MODEL', 'CODEX_OCTO_SDK_ALLOWED_TOOLS', 'CODEX_OCTO_SDK_PERMISSION_MODE',
  'CODEX_OCTO_SDK_MAX_TURNS', 'CODEX_OCTO_SDK_SYSTEM_PROMPT', 'CODEX_OCTO_SDK_SETTING_SOURCES',
  'CODEX_OCTO_RATE_LIMIT_MAX_PER_MINUTE', 'CODEX_OCTO_CONTEXT_MAX_CHARS',
  'CODEX_OCTO_CONTEXT_HISTORY_LIMIT', 'CODEX_OCTO_BOT_BLOCKLIST',
  'CODEX_OCTO_MENTION_FREE_GROUPS', 'CODEX_OCTO_MAX_RESPONSE_CHARS',
  'ANTHROPIC_BASE_URL', 'CODEX_OCTO_SDK_TOOL_PROGRESS',
  'CODEX_OCTO_GROUP_CONFIG_DIR',
  'CODEX_OCTO_TRANSPORT', 'CODEX_OCTO_WEBHOOK_HOST', 'CODEX_OCTO_WEBHOOK_PORT',
  'CODEX_OCTO_WEBHOOK_PATH', 'CODEX_OCTO_WEBHOOK_SECRET',
  'CODEX_OCTO_MEMORY_BASE',
];
...
describe('loadConfig defaults', () => {
  ...
  it('returns correct shared defaults; baseDir = config dir; dirs derived per bot', () => {
    ...
    // Codex runtime defaults: safe sandbox + headless approval + medium effort.
    expect(cfg.sdk.sandboxMode).toBe('read-only');
    expect(cfg.sdk.allowWorkspaceWrite).toBe(false);
    expect(cfg.sdk.approvalPolicy).toBe('never');
    expect(cfg.sdk.modelReasoningEffort).toBe('medium');
    expect(cfg.sdk.networkAccessEnabled).toBe(false);
    expect(cfg.sdk.webSearchEnabled).toBe(false);
    expect(cfg.sdk.webSearchMode).toBe('disabled');
    expect(cfg.sdk.codexBaseUrl).toBeUndefined(); // unset by default
...
describe('Config file permission warning (Q12)', () => {
  ...
  beforeEach(() => {
    ...
    process.env.CODEX_OCTO_BOT_TOKEN = 'test-token';
    process.env.CODEX_OCTO_API_URL = 'https://test-api';
  });

  afterEach(() => {
    delete process.env.CODEX_OCTO_BOT_TOKEN;
    delete process.env.CODEX_OCTO_API_URL;
    rmSync(tmpDir, { recursive: true, force: true });
  });
...
// ─── codexBaseUrl (Codex upstream override) ────────────────────────────

describe('codexBaseUrl', () => {
  beforeEach(setup);
  afterEach(teardown);

  it('reads sdk.codexBaseUrl from config file', () => {
    const path = writeConfig({
      botToken: 'bf_t',
      apiUrl: 'https://a',
      sdk: { codexBaseUrl: 'https://gw.example.com' },
    });
    expect(loadConfig(path).sdk.codexBaseUrl).toBe('https://gw.example.com');
  });

  it('rejects an http:// (non-localhost) codexBaseUrl (SSRF protection)', () => {
    const path = writeConfig({
      botToken: 'bf_t',
      apiUrl: 'https://a',
      sdk: { codexBaseUrl: 'http://evil.example.com' },
    });
    expect(() => loadConfig(path)).toThrow(/Unsafe sdk\.codexBaseUrl/);
  });

  it('rejects a codexBaseUrl resolving to a private IP literal', () => {
    const path = writeConfig({
      botToken: 'bf_t',
      apiUrl: 'https://a',
      sdk: { codexBaseUrl: 'https://169.254.169.254' },
    });
    expect(() => loadConfig(path)).toThrow(/Unsafe sdk\.codexBaseUrl/);
  });
  ...
  it('allows http://localhost for local development', () => {
    const path = writeConfig({
      botToken: 'bf_t',
      apiUrl: 'https://a',
      sdk: { codexBaseUrl: 'http://localhost:8080' },
    });
    expect(loadConfig(path).sdk.codexBaseUrl).toBe('http://localhost:8080');
  });
});
...
describe('sandboxMode', () => {
  ...
  it('config file can set sandboxMode to workspace-write', () => {
    const path = writeConfig({
      botToken: 'bf_t',
      apiUrl: 'https://a',
      sdk: { sandboxMode: 'workspace-write' },
    });
